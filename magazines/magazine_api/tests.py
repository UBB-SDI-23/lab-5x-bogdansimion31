...
from .models import Author, Magazine, Buyer, Purchase, Publisher, PublisherStatsDTO
from .serializer import MagazineSerializer, PublisherStatsDTOSerializer
...
class NonCrudFunctionTests(TestCase):
    def setUp(self):
        ...
        self.magazine1 = Magazine.objects.create(title="Test Magazine 1", number_of_pages=100, publish_date="2022-01-01", quantity=10, ibn=12345, author=self.author, publisher=self.publisher1, price=100)
        self.magazine2 = Magazine.objects.create(title="Test Magazine 2", number_of_pages=200, publish_date="2022-02-01", quantity=20, ibn=12346, author=self.author, publisher=self.publisher2, price=200)
    ...
    def test_magazines_with_pages_above(self):
        min_pages = 150
        response = self.client.get(reverse('magazines_with_pages_above', kwargs={'min_pages': min_pages}))
        self.assertEqual(response.status_code, 200)

        # Filter magazines with number_of_pages greater than or equal to min_pages
        magazines = Magazine.objects.filter(number_of_pages__gte=min_pages)
        expected_response_data = MagazineSerializer(magazines, many=True).data
